...
import {createRaceWithResultsSchema} from '@/schemas/raceWithResultsSchemas'
import {createRaceWithResultsAction} from '@/serverFunctions/raceWithResults'
...
const ResultsForm: FunctionComponent<ResultsFormProps> = ({circuits, drivers, seasons}) => {
  const [form, createRaceWithResults] = useZodValidatedForm(createRaceWithResultsSchema, createRaceWithResultsAction, {
    defaultValues: {
      name: '',
      circuitId: '',
      seasonId: '',
  ...
  return (
    <Form hookForm={form} className="space-y-6" action={createRaceWithResults}>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge variant="secondary">{fields.length} Drivers</Badge>
              </div>
            </CardHeader>
              ...
              {fields.map((field, index) => {
                ...
                return (
                  <div key={field.id} className="flex items-start gap-4 rounded-lg border p-4">
                    {/* Hidden inputs voor position en points */}
                    <div className="hidden">
                      <input type="hidden" {...form.register(`results.${index}.position`)} value={position} />
                      <input type="hidden" {...form.register(`results.${index}.points`)} value={points} />
                    </div>
